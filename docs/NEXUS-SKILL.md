...
TaskFlow es tu sistema de gestión — aquí vive todo: tareas, seguimientos de temas, contexto de reuniones/emails, recordatorios y tu memoria profesional sobre Pedro.
...
Los seguimientos son los TEMAS que Pedro está gestionando. Cada tema acumula entradas de contexto, tareas vinculadas y recordatorios.
...
# Ver detalle de un seguimiento (incluye items, entradas, recordatorios)
...
Las entradas son PIEZAS DE INFORMACIÓN vinculadas a seguimientos: emails, notas de reunión, conversaciones, etc. Cuando creas una entrada, Nexus (en modo mock/local) la procesa automáticamente con el LLM, extrae acciones, crea tareas y recordatorios.
...
**IMPORTANTE**: Al crear una entrada, el sistema automáticamente:
- Extrae acciones → crea Items (tareas)
...
curl -X PATCH "$TASKFLOW_URL/api/items/{id}" \
  ...
  -d '{
    "estado": "IN_PROGRESS",
    "notasAgente": "Nexus: Movido a en progreso porque Pedro lo mencionó en Telegram",
...
4. El sistema procesará automáticamente y creará tareas/recordatorios si aplica
...
3. `GET /api/seguimientos` y filtrar los relevantes
